...
When you run the CLI locally from a project directory, shell commands (including `pwd`) and coding-agent work use that directory automatically:
...
This requires the Nakama server to run on the same machine. The profile workspace still stores Nakama soul, memory, and skills.
