...
On web chat, `write_file` and `write_docx` saves under `artifacts/` appear as attachment chips on the assistant message. Click a chip to open a resizable preview panel. Copy, download, and fullscreen are always available. Markdown and HTML have a Preview / source toggle: source is where you edit, then **Save**. Preview still shows the rendered file, including unsaved edits. JSON and other text files use **Edit** instead — they have no preview toggle. Word previews stay read-only.
