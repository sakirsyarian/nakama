...
On web chat, `write_file` and `write_docx` saves under `artifacts/` appear as attachment chips on the assistant message. Click a chip to open a resizable preview panel. Copy, download, and fullscreen are always available. Markdown and HTML have Preview, source, and **Edit** tabs; **Edit** opens a text editor, then **Save**. Markdown preview replaces the file name with a contents menu when the file has more than one heading. JSON and other text files keep **Edit** in the action bar. Word previews stay read-only.

Ask the agent to revise an artifact after you edit it in the panel — it reads the current file from disk, then updates that same path. It cannot delete files under `artifacts/` from chat; that would break older chips. Remove files from the Artifacts tab instead.
...
|--------------|------------------|
| HTML (`.html`, `text/html`) | Sandboxed iframe render; editable as source |
| Markdown (`.md`, `text/markdown`) | Rendered prose; editable as source |
| Word (`.docx` from `write_docx`) | Server converts to Markdown for preview (not editable) |
| JSON, code, plain text | Syntax highlighting or monospace block; editable |
...
**Restrictions:** Rejects `.docx` and `.doc` paths — a `.docx` is a ZIP archive, not UTF-8 text. Use `write_docx` instead. Under `artifacts/`, existing files are not overwritten; a date suffix is added instead.
...
**Restrictions:** Refuses paths under `artifacts/` so chat attachment chips keep working. Revise artifacts with `read_file` then `edit_file`. Remove files from the Artifacts tab.
...
**Execution backend:** By default bash runs on the Nakama **host** (`NAKAMA_BASH_BACKEND=host`). Set `NAKAMA_BASH_BACKEND=microsandbox` to run each profile in its own warm MicroSandbox microVM (1 profile → 1 sandbox) with the profile workspace bind-mounted. Defaults: public network **off** (`NAKAMA_BASH_SANDBOX_NETWORK=off|public`), image `alpine` (`NAKAMA_BASH_SANDBOX_IMAGE`). If MicroSandbox is selected but unavailable, the tool **fails closed** — it does not fall back to host. `codingAgent: true` / harness CLI runs are **unsupported** under `microsandbox` (use `host` for coding-agent). Stock Docker without `/dev/kvm` cannot enable MicroSandbox — see [Docker](/docker).

#### Choose a MicroSandbox image

The default Alpine image has no Python. To give profiles Python, start Nakama with a Python image:

```bash
NAKAMA_BASH_BACKEND=microsandbox NAKAMA_BASH_SANDBOX_IMAGE=python:3.12 bun run dev:server
```

To include your own packages, create a `Dockerfile` in an empty directory:
...
Build it and load it into MicroSandbox on the machine running Nakama:

```bash
docker build -t nakama-bash:python-git .
docker save nakama-bash:python-git | msb load
NAKAMA_BASH_BACKEND=microsandbox NAKAMA_BASH_SANDBOX_IMAGE=nakama-bash:python-git bun run dev:server
```

Use the same image name when starting Nakama. For a Docker deployment, publish the image to a registry accessible to the Nakama container and set `NAKAMA_BASH_SANDBOX_IMAGE` to its registry name; a MicroSandbox cache loaded on the Docker host is not the container's cache. The first profile run may wait while MicroSandbox pulls a registry image. See [MicroSandbox image commands](https://docs.microsandbox.dev/cli/image-commands) for `msb load` and `msb pull`.
