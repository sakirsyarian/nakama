...
For a container or secrets manager, keep the key outside Nakama's data directory. Mount it as a readable file and set:

```sh
COMPOSIO_API_KEY_FILE=/run/secrets/composio_api_key
```

You can also set `COMPOSIO_API_KEY` directly. The direct value takes priority over the mounted file, and either one takes priority over a key saved through **Integrations**.
...
- An API key saved through **Integrations** is stored as `api_key` in `~/.nakama/composio/config.ini` on the server. `COMPOSIO_API_KEY` and `COMPOSIO_API_KEY_FILE` keep it outside that file.
