...
- **Stale (30 days by default):** listed in the last report. Files stay in place. Org admins can choose a different stale interval.
- **Archive (90 days by default):** the skill folder is **moved**, never deleted, to `~/.nakama/orgs/{orgId}/profiles/{profileId}/skills/.archive/` (or `$NAKAMA_CONFIG_DIR/orgs/...` if you set that). The skill is unassigned so it leaves the chat catalog. Org admins can choose a different archive interval. Restore UI is not in this release: move the folder back to `skills/`, then assign it on the profile again.
- **Unused means match or patch, not a catalog view:** the configured stale/archive clock uses last **match** or last **update**. Catalog views do not reset it.
...
- Set **Freshness clocks**. Defaults are 30 days stale and 90 days archive; stale must be before archive.
- Set **Automation worker poll interval** if this installation needs a different schedule reload cadence. It is workspace-global and defaults to 5 minutes.
