...
Creating or changing an automation or a task requires a non-viewer role. A caller-supplied `profileId` on create or update is checked against what that caller may use, so a member cannot bind an automation to the `bash`-capable Super Bot profile. See [Multi-tenancy](/multi-tenancy) for the role model.
