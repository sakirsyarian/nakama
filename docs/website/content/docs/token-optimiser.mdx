...
description: "Shorten bash, read_file and knowledge_base_search output before it reaches the model, measure what that saves, and expand anything folded."
---
...
Three tools: `bash` (its `stdout`), `read_file` (its `content`) and `knowledge_base_search` (its `matches`). Output under 1,000 characters is passed through untouched.
...
`knowledge_base_search` folds a much smaller share of its own volume, 9.6% measured over 31 sessions of a retrieval agent, and all of it is whole results repeated within a session rather than partial overlap. It is in anyway because of the base rather than the ratio: an agent that answers from uploaded documents often has no shell at all, so the panel read `Nothing measured in the last 30 days` forever and looked identical to a broken install. A folded search result keeps its `matchCount` and drops its hits, which the handle expands.
...
- `bash`, `read_file` and `knowledge_base_search` only.
